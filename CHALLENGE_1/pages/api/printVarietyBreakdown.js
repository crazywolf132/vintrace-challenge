...
export default (req, res) => {
	const temp = extractUnique(overAllData, 'region');

	const final = temp.map((company) => {
		let unique = 0;
		company.region.forEach((type) => {
			...
			if (_.filter(temp, { region: [type] }).length === 1) {
				// This means its unique, as only 1 item in the list of companies has it AKA (this company)
				unique++;
			}
		});
		company.uniquePercentage = (unique / company.region.length) * 100;
